...
class Plugin(AbstractPlugin):
    ...
    def get_location(self, parsed):
        ...
        # IF A LOCATION IS INCLUDED IN THE QUERY, USE THE LOCATION NAME AS THE LOCATION
        if len(words) > 0:
            return words[0]
        # IF THE LOCATION IS NOT INCLUDED, BUT THE CONFIGURATION FILE SPECIFIES LOCATION, USE LOCATION
        else:
            return config.get('location', '上海')
    ...
    def handle(self, text, parsed):
        ...
        try:
            ...
            if 'results' in weather:
                ...
                self.say('无法获取天气数据，请稍后重试', cache=True)
