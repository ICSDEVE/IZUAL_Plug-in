...
#
# @Author  : Jehovah
# @File    : RaspberryPiStatus.py
# @Time    : 09/25/21 22:49:29
# @Version : v1.0
#
...
class Plugin(AbstractPlugin):
    ...
    def getCPUtemperature(self):
        ...
        try:
            ...
            self.say(u'无法获取CPU温度', cache=True)
        return result
    ...
    def handle(self, text, parsed):
        try:
            ...
            self.say(u'CPU温度' + str(status['cpu_tmp']) + u'度, 内存使用百分之' + str(status['ram_percentage']) + u', 存储使用百分之' + str(status['disk_percentage']))
        except Exception as e:
            logger.error(e)
            self.say(u'无法获取树莓派状态', cache=True)
