...
"""
SAVE THE PERSONAL INFORMATION OF NETEASEMUSIC USERS IN A JSON FILE
"""
class Storage(object):
    ...
    def load(self):
        try:
            ...
            logger.error('加载database.json文件失败！原因是{}'.format(e))
        self.save()
...
"""
FOR NETEASE CLOUD PLAYER PLUG-IN,
ON THE BASIS OF MUSICPLAYER, IT SUPPORTS LOGGING IN AND CONNECTING TO NETEASE CLOUD API TO RETURN RELEVANT INFORMATION
"""
class NeteaseMusicPlayer(MusicPlayer):
    ...
    def __init__(self, playlist, plugin, **kwargs):
        ...
        # YOU NEED TO INITIATE A REQUEST TO GET COOKIES WHEN YOU LOG IN FOR THE FIRST TIME,
        # AND YOU NEED TO INITIATE A REQUEST WHEN COOKIES EXPIRE OR YOU CAN’T FIND THE USER ID
    ...
    def login(self):
        ...
        if resp["code"] == 200:
            ...
            self.plugin.say('Login successful！', cache=True, wait=True)
            return True
        else:
            self.plugin.say('Failed to login', cache=True, wait=True)
            logger.error('Status code：{}'.format(resp['code']))

    def daily_checkin(self):
        res = self.api.daily_task(is_mobile=False)
        if res["code"] == 200:
            self.plugin.say('已帮你悄悄的签到了~', cache=True, wait=True)
    ...
    def play(self):
        ...
        # BECAUSE THE PLAY AND NEXT METHODS OF THE MUSICPLAYER OBJECT ASSUME THAT THE TIMELINESS OF SELF.PLAYLIST IS PERMANENT,
        # REFACTOR THE PLAY METHOD TO CHECK THE TIMELINESS OF THE PLAYLIST
        ...
        if 'song/media/outer' in path:
            logger.info('{}--{}怀疑未获取到版权, 你可以用"play {}"命令试试看~'.format(self.playlist[self.idx]['artist'], self.playlist[self.idx]['song_name'], self.playlist[self.idx]['mp3_url']))
            self.next()
        else:
            logger.info('目前播放的歌曲是{}, 歌手为{}'.format(self.playlist[self.idx]['song_name'], self.playlist[self.idx]['artist']))
    ...
    def get_search_result(self, singerName=None, songName=None):
        if songName:
            ...
            if not data['songCount']:
                return None
            # OPTIMIZE THE JUDGMENT LOGIC (BECAUSE THE SINGER NAME AND SONG NAME CANNOT BE SEPARATED WELL)
            # 例：播放许飞的父亲写的散文诗
            #       singerName: 许飞的父亲写
            #       songName: 散文诗
            if singerName:
                datalist = None
                # IF THERE IS AN ARTIST NAME, GET THE SONG OF THE CURRENT ARTIST ACCURATELY
                for song in data['songs']:
                    if singerName == song['artists'][0]['name']:
                        datalist = self.api.dig_info([song], 'songs')
                        break
                # IF YOU CAN’T EXACTLY MATCH THE SINGER’S NAME, GET THE FIRST SONG
                if not datalist:
...
"""
This type mainly deals with the user’s text input request,
Strip the parameters in the request and pass it to NeteaseMusicPlayer for processing
"""
class Plugin(AbstractPlugin):
    ...
    def handle_multilists(self, text):
        # ASK THE USER FOR MULTIPLE ROUNDS OF THE PLAYLIST HE WANTS TO CHOOSE, AND KEEP LOOPING UNTIL HE GETS THE ANSWER OR SHOUTS "NO NEED"
        ###################################ASK FOR THE NAME OF A SPECIFIC PLAYLIST###########################################
        if any(word in text for word in [u'啥', u'什么', u'叫']) and hasNumbers(text):
            ...
            if listNumber <= len(self.player.multi_listChoices):
                ...
                self.say('都说了只有{}张啦~'.format(len(self.player.multi_listChoices)), onCompleted=lambda: self.handle_multilists(self.activeListen()))
        ###################################INDICATES THAT I WANT TO CONTINUE LISTENING TO THE NEXT ROUND###########################################
        ...
        ###################################MEANS TO RE-BROADCAST THE PLAYLIST###########################################
        ...
        ###################################SELECT A PLAYLIST###########################################
            ...
            if listNumber >= len(self.player.multi_listChoices):
                ...
                if self.player.playlist:
                    ...
                    self.say('无法获取相关信息！', wait=True)
        ###################################OPT OUT OF THIS LOOP###########################################
        elif any(word in text for word in [u"不要", u"算了", u"不用"]):
            self.playlist_number_cut = 1
            self.isContinueAsking = False
            self.say('哼！白浪费帮你找来了这么多的资源！', cache=True)
        ###################################WHEN SAID SOMETHING ELSE###########################################
        else:
            self.say('别岔开话题，到底想听哪张的呀~', cache=True, onCompleted=lambda: self.handle_multilists(self.activeListen()))

    def handle_search(self, input):
        # MULTIPLE ROUNDS OF ASKING USERS IF THEY WANT TO SEARCH FOR THIS SONG/SINGER, KEEP LOOPING UNTIL THEY GET THE ANSWER
        ###################################ASK IF THE NAME OF THE SONG OR SINGER JUST IDENTIFIED IS CORRECT###########################################
        ...
        ###################################EXTRACT THE SINGER OR SONG NAME, IF YOU CAN’T EXTRACT IT, ASK AGAIN###########################################
        else:
    ...
    def handle(self, text, parsed):
        # NEED TO CONFIGURE RELEVANT INFORMATION FOR THE NETEASE CLOUD PLUG-IN
        profile = config.get()
        ...
        try:
            # PLUG-IN CORE PROCESSING LOGIC
            ###################################IF THE USER WANTS TO SEARCH FOR SINGERS/SONG TITLES/SONG TITLES WITH SINGERS, SO WILL HAVE THIS INTENTION####################################
            if self.isSearch:
                self.handle_search(text)
            ###################################INTERRUPTED WHEN BROADCASTING PLAYLIST INFORMATION, DIRECTLY ENTER MULTIPLE ROUNDS OF INQUIRY###########################################
            elif self.isContinueAsking:
                self.handle_multilists(text)

            ###################################HAVE NOT PLAYED THE SONG YET, GET THE USER’S DAILY RECOMMENDED SONG###########################################
            elif any(word in text for word in ['推荐歌曲', '推荐的歌曲', '歌推荐', '每日推荐']):
                self.player.set_playlist()
                self.isOpenLikelist = False
                if self.player.playlist:
                    self.say('今天共有{}首推荐歌曲！'.format(len(self.player.playlist)), wait=True)
                    self.player.play()
                else:
                    self.say('无法获取相关的每日推荐歌曲！', cache=True, wait=True)

            ###################################GET DAILY RECOMMENDED PLAYLIST##############################################################
                ...
                if not self.player.multi_listChoices:
                    self.say('找不到任何推荐歌单！你是不是很少用网易云~', cache=True, wait=True)
                else:
                    logger.info(playlists_info)
                    self.playlist_number_cut += 1
                    self.isContinueAsking = True
                    self.say('共找到了{}张歌单！'.format(len(self.player.multi_listChoices)) + playlists_info + '想听哪一张，或者要不要继续听下去呢', onCompleted=lambda: self.handle_multilists(self.activeListen()))

            ###################################GET THE USER’S RED HEART PLAYLIST (USER’S FAVORITE MUSIC)###########################################
            elif any(word in text for word in ['红心歌单', '红星歌单', '喜欢的音乐']):
                self.player.set_playlist(isLikeList=True)
                if self.player.playlist:
                    self.isOpenLikelist = True
                    self.say('红心歌单共有{}首歌曲！'.format(len(self.player.playlist)), wait=True)
                    self.player.play()
                else:
                    self.say('无法获取你的红心歌单！', cache=True, wait=True)
            ...
            ###################################GET MY PLAYLIST####################################
                ...
                if not self.player.multi_listChoices:
                    self.say('找不到任何你的歌单！你是不是很少用网易云~', cache=True, wait=True)
                elif len(self.player.multi_listChoices) > 5:
                    self.playlist_number_cut += 1
                    self.isContinueAsking = True
                    self.say('你一共有{}张歌单！'.format(len(self.player.multi_listChoices)) + playlists_info + '想听哪一张，或者要不要继续听下去呢', onCompleted=lambda: self.handle_multilists(self.activeListen()))
                else:
                    self.isContinueAsking = True
