...
class Plugin(AbstractPlugin):
    ...
    def handle(self, text):
        ...
        if self.SLUG not in profile or \
            ...
            self.say('远控插件配置错误，激活插件失败', cache=True)
            return
        self.pc_email = profile[self.SLUG]['pc_email']
        try:
            if any(word in text for word in [u"关机", u"关电脑", u"关闭电脑"]):
                self.say('即将关闭电脑，请在提示音后进行确认', cache=True, onCompleted=lambda: self.shutdown(self.activeListen()))
                ...
                self.say('即将发送快捷键%s，请在提示音后进行确认' % self.button, cache=True, onCompleted=lambda: self.sendHotKey(self.activeListen()))
                ...
                self.say('即将发送指令%s，请在提示音后进行确认' % self.cmd, cache=True, onCompleted=lambda: self.command(self.activeListen()))
        except Exception as e:
            logger.error(e)
            self.say('指令发送失败', cache=True)
